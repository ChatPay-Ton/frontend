...
const ClientContracts: React.FC = () => {
  ...
  const { contracts, loading, error, refetch, contractsCount } = useClientContracts();
          ...
          {/* {contractsCount > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-blue-50 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-blue-600">{contracts.filter(c => c.status === 'created').length}</div>
                <div className="text-xs text-blue-700">Criados</div>
              </div>
              <div className="bg-yellow-50 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-yellow-600">{contracts.filter(c => c.status === 'deposited').length}</div>
              ...
              <div className="bg-purple-50 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-purple-600">
                  {contracts.reduce((total, contract) => total + contract.total_amount, 0).toFixed(2)}
                </div>
                <div className="text-xs text-purple-700">TON Total</div>
              </div>
            </div>
          )} */}
        </div>
